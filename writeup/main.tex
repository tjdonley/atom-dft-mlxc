...
invariants in three ways --- the dimensionless gradient (and Laplacian) ingredients
of the gradient expansion, the power spectrum and bispectrum that form a complete
basis, and targeted projections of specific physical operators.
...
density functional~\cite{Antoniewicz_1985,Perdew_1996}. Its ingredients, the reduced
gradient $s=|\nabla\rho|/(2\kF\rho)$ and reduced Laplacian $q=\nabla^2\rho/(4\kF^2\rho)$
(with $\kF=(3\pi^2\rho)^{1/3}$), are explicit contractions of the SIMPLE features:
$\nabla\rho$ is a vector (rank $1$) and $\nabla^2\rho$ a scalar (rank $0$), so they
live in the $\ell=1$ and $\ell=0$ channels,
\begin{equation}
\label{eq:sq}
...
factors, so no empirical decoder is needed; the reduced gradient is just the $\ell=1$
power of the spectrally-weighted features and inherits their rotational and scale
invariance.

The reduced gradient is well-behaved and is the ingredient the functional uses
(Sec.~\ref{sec:hole}): within the basis resolution it agrees with direct finite
differences to $\sim1\%$ (Sec.~\ref{sec:results-cartesian}), feeding the exact
second-order coefficient $\mu=10/81$. The reduced Laplacian is reconstructable by the
same construction but \emph{not numerically usable}: like a standard finite-difference
...
dense environments (Sec.~\ref{sec:results-cartesian}, App.~\ref{app:gea}). We
therefore carry the expansion only to second order in $s$ and exclude $q$.
...
increase. Alternatively one can \emph{target a specific operator} whose symmetry is
known and project it directly onto the features.
...
semilocal functionals miss --- it is \emph{not} merely the average density. The same
contraction with a model exchange hole in place of the bare kernel yields the
self-Coulomb $\Phi_S$ of Eq.~\eqref{eq:eps-x}, the basis for the functional below.
...
On the 3D Cartesian systems the invariants are stable under the symmetry operations a
functional must respect (Fig.~\ref{fig:cartesian}; full breakdown in
App.~\ref{app:stress}). Under random rotations the rotation-invariant features ($s$,
$\tilde P$, the bispectrum, and $q$) hold to $\le1.4\%$; under sub-grid translation
they stay $\le1\%$ except at the bare pseudo-H$_2$ cusp ($6.5\%$); and the power
spectrum and bispectrum converge as $\mathcal O(h^2)$ toward the continuum ($\le6\%$
at $h=0.2$). Uniform scaling is the one resolution-gated property: deviations grow
once $\lambda$ exceeds $\Lambda_{\max}=\nin/\nout$ but are recovered by raising $\nin$
(App.~\ref{app:stress}), not a breakdown of the construction. The reduced Laplacian
$q$ is the lone casualty: its reconstruction error reaches tens to hundreds of times
the value in dense bulk environments, confirming that $q$ cannot be carried.
...
\caption{Invariance of the SIMPLE invariants on 3D Cartesian grids ($\Rc=6$~bohr):
worst-case relative deviation of each feature family ($s$, $q$, power spectrum
$\tilde P$, bispectrum) under rotation, sub-grid translation, uniform scaling, and
grid refinement, maximized over the five test systems; dashed line is the $10\%$ flag.
Rotation, translation, and grid refinement are well controlled for $s$, $\tilde P$,
and the bispectrum; uniform scaling is resolution-gated (recoverable by raising $\nin$,
App.~\ref{app:stress}); the reduced Laplacian $q$ is broadly flagged. Detailed
per-test panels are in App.~\ref{app:stress}.}
\label{fig:cartesian}
\end{figure}
