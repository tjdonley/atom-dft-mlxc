...
$\ell=1$ kernel node --- which we pin by the Lieb--Oxford bound (C7)~\cite{Lieb_1981,Perdew_1996}:
the width is chosen so the realized slowly-varying enhancement peaks exactly at the ceiling
$F_x=1.804$ (Fig.~\ref{fig:fx}), with the node amplitude re-solved to hold the $10/81$ slope. Along
...
value $\mu=10/81$ [Eq.~\eqref{eq:fx}]. The $\ell=1$ node width is pinned by the Lieb--Oxford bound:
the enhancement rises to touch the ceiling $F_x=1.804$ (dotted) exactly once, at its peak
($s\approx3.3$, marker), then relaxes back toward LDA --- in contrast to the raw pointwise GEA (grey
dashed), which diverges, and PBE~\cite{Perdew_1996} (black), which saturates monotonically. The
on-top value is preserved for any amplitude, so the Fermi--Amaldi limit is untouched. On real atoms
the $\ell=0$ power-spectrum gate keeps the realized enhancement near LDA (App.~\ref{app:kernel}).}
\label{fig:fx}
\end{figure}
...
Lieb--Oxford bound (C7) supplies the last free number --- the $\ell=1$ kernel width is fixed so
$F_x$ touches the ceiling $1.804$ at its peak (Fig.~\ref{fig:fx}); those requiring
...
parameters: the kernel is anchored at the homogeneous-gas node (LDA), its single
$\ell=1$ node amplitude is fixed by the gradient expansion [Eq.~\eqref{eq:fx}], and that
node's width by the Lieb--Oxford bound (C7). We
