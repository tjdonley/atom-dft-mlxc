...
sharpens the strongly inhomogeneous region while leaving the LDA, Fermi--Amaldi, and gradient
...
exchange to a mean absolute error of $34$~mHa across the fourteen closed-shell atoms from He to
Hg, \emph{below} PBE ($45$~mHa) and approaching rSCAN ($14$~mHa, which additionally consumes the
kinetic-energy density) (Table~\ref{tab:ref-energies}). The noble-gas and filled-$d$ atoms are
reproduced to $\sim10$~mHa (Ne $11$, Ar $10$, Kr $6$, Pd $4$); the residual error concentrates in
the alkaline earths (Be, Ca, Sr, Ba), the low-enclosed-charge $ns^2$ shells where the
Fermi--Amaldi channel dominates the hole. The reference coverage of the density manifold, rather
than the descriptor resolution, sets this accuracy. Self-consistency and the full periodic table,
deferred in earlier versions, are addressed next (Sec.~\ref{sec:results-allatom}).
...
\begin{table}[tb]
\centering
\caption{Closed-shell atomic exchange errors (mHa) of the \emph{referenced} SIMPLE kernel hole,
evaluated on the exact density, versus exact exchange, with PBE~\cite{Perdew_1996} and
rSCAN~\cite{Bart_k_2019}. The kernel is populated with exact holes covering the closed-shell density
manifold ($\rho\gtrsim10^{-2}$). SIMPLE's mean absolute error ($34$~mHa) is below PBE ($45$); rSCAN
($14$) additionally uses the kinetic-energy density. The residual concentrates in the alkaline-earth
$ns^2$ shells (Fermi--Amaldi-dominated).}
...
\paragraph*{Exchange energies across the table.} We compare the self-consistent exchange energy of the
functional to the \emph{Wigner-subtracted} Hartree--Fock reference --- the spherically-averaged exact
...
\begin{table}[tb]
\centering
\caption{Self-consistent exchange-energy errors (mean absolute error, mHa) of the production SIMPLE kernel
hole against the Wigner-subtracted (spherically-averaged) exact exchange, with PBE~\cite{Perdew_1996} and
