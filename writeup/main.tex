...
(App.~\ref{app:kernel}). Fixing the slope leaves a single open length scale --- the width of the
$\ell=1$ kernel node --- a free parameter held at a moderate value. The resulting slowly-varying
...
holding the $10/81$ slope. Along
the $\ell=0$ (power-spectrum) axis the kernel still interpolates a deviation that decays away from
the HEG signature, so on real atoms --- whose local power spectra depart from the homogeneous gas ---
the enhancement remains a \emph{bounded deformation about} LDA. Reference-free this leaves the
functional close to LDA on atoms; strongly inhomogeneous (open-shell) atoms are
...
($\approx1.05$) and relaxes back, so reference-free the hole is essentially LDA+Fermi--Amaldi.
...
Fermi--Amaldi limit is untouched.}
\label{fig:fx}
\end{figure}
...
Lieb--Oxford bound (C7) is respected (the enhancement stays below $1.804$; Fig.~\ref{fig:fx}); the one
remaining length scale, the $\ell=1$ kernel width, is a free parameter. Those constraints requiring
