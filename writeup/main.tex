...
monopole \emph{directly} in the scale-free SIMPLE radial basis,
\begin{equation}
\label{eq:hole-direct}
\bar n_x(r_0,u)=\sum_n \tilde\varrho_{n}(r_0)\,R_{n0}(u/\Rad),
\end{equation}
so the hole is a coefficient vector $\tilde{\bm\varrho}$ on the adaptive unit frame $u=\Rad t$
[$\Rad\kF=\xi^*$, Eq.~\eqref{eq:adaptive}]. The $1/u$ weight then collapses the energy per electron
to a single linear contraction onto the basis Coulomb moments,
\begin{equation}
\label{eq:eps-direct}
\varepsilon_x(r_0)=2\pi\Rad^2\,\tilde{\bm\varrho}\!\cdot\!\bm\beta,\qquad
\beta_{n}=\!\int_0^1\!R_{n0}(t)\,t\,dt ,
\end{equation}
and the two exact constraints are likewise linear: the sum rule
$4\pi\Rad^3\,\tilde{\bm\varrho}\!\cdot\!\bm\alpha=-1$ ($\alpha_{n}=\int_0^1 R_{n0}(t)\,t^2dt$) and
the on-top value $\tilde{\bm\varrho}\!\cdot\!\mathbf R_0=-\tfrac12\rho(r_0)$ [$R_{0,n}=R_{n0}(0)$].
Only the $\bm\beta$-projection of the hole reaches the energy.
...
homogeneous-gas (LDA) limit $F_x\to1$; (C5)~the Fermi--Amaldi one-electron limit; (C6)~the
...
a parameter-free map that meets (C1)--(C7) by construction.
...
onto the basis and \emph{moment-matched} to its exact charge, on-top, and Coulomb moments, which
makes $F_x=1$ hold exactly at finite basis size (App.~\ref{app:hole}); this LDA hole
$\bm\sigma_{\mathrm{LDA}}$ anchors the map. Where the window cannot enclose a spin pair --- a
one-electron core or density tail, per-spin charge $Q_\sigma<1$ --- no screened scale exists and the
hole becomes the density-following $-\mathbf C/Q$ (the negative neighboring density, with monopole
coefficients $\mathbf C$ and enclosed charge $Q=4\pi\Rad^3\,\tilde{\bm\varrho}\!\cdot\!\bm\alpha$),
the exact one-electron self-interaction correction (C5); a smooth $C^2$ charge gate
$W_{\mathrm{FA}}(Q_\sigma)$ blends the two, reproducing H and spin-paired He to $\sim2$~mHa. Forming
...
Inhomogeneity enters only through the $\ell=1$ SIMPLE features, whose squared distance from the
uniform gas is the reduced gradient $s^2$ [Eq.~\eqref{eq:sq}]. We produce the bulk coefficients by
interpolating the scale-free hole \emph{shape} $\bm\sigma=\tilde{\bm\varrho}/(-\tfrac12\rho)$ over
fixed points with a kernel,
\begin{equation}
\label{eq:kernel-map}
\bm\sigma(\mathbf x)=\bm\sigma_{\mathrm{LDA}}+\sum_k \mathcal K(\mathbf x,\mathbf x_k)\,\mathbf c_k ,
\end{equation}
where $\mathbf x$ collects the per-$\ell$ SIMPLE distances ($\ell=0$: the monopole power vector;
$\ell=1$: $s^2$) and the HEG node $\mathbf x=0$ pins $\bm\sigma=\bm\sigma_{\mathrm{LDA}}$. Because
the energy is linear in $\tilde{\bm\varrho}$ [Eq.~\eqref{eq:eps-direct}], the enhancement is a
linear functional of a single moment of the shape deviation
$\delta\bm\sigma=\bm\sigma-\bm\sigma_{\mathrm{LDA}}$,
\begin{equation}
\label{eq:Fx-kernel}
F_x-1=\kappa\,\delta\bm\sigma\!\cdot\!\bm\beta,\qquad
...
(App.~\ref{app:kernel}). The expansion is smoothly capped at the Lieb--Oxford ceiling
$F_x\le1.804$ (C7)~\cite{Lieb_1981,Perdew_1996}, a parameter-free saturation. Strongly inhomogeneous
(open-shell) atoms are improved by adding exact atomic holes as further kernel fixed points: each
carries only the deviation beyond LDA and vanishes far from its reference, so the LDA, Fermi--Amaldi,
and gradient limits are untouched (App.~\ref{app:kernel}).

\begin{figure}[tbp]
\centering
\includegraphics[width=\columnwidth]{figures/fx_enhancement.pdf}
\caption{Exchange enhancement factor of the scale-free SIMPLE hole. The $\ell=1$ kernel channel
sets the small-$s$ slope to the exact second-order value $\mu=10/81$ [Eq.~\eqref{eq:fx}]; the
enhancement is then smoothly saturated, with the ceiling fixed by the local Lieb--Oxford bound
$F_x\le1.804$ (dotted). The realized enhancement (solid, the re-summed hole
$F_x=\varepsilon_x/\varepsilon_x^{\mathrm{unif}}$ on the homogeneous gas at the adaptive window)
tracks the exact slope at small $s$, then saturates below the LO bound where the raw pointwise GEA
(grey dashed) diverges --- a PBE-like~\cite{Perdew_1996} (black) saturation arising here from the
Lieb--Oxford ceiling rather than a fitted parameter. The on-top value is preserved for any
...
Constraints (C1)--(C7) are thus satisfied by construction rather than by fitting; those requiring
...
response of Eq.~\eqref{eq:fx}, not fit to any atomic data --- on a single cutoff
