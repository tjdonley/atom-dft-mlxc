...
Holes are now SPIN-RESOLVED for every atom: `orbital_hole.exchange_hole_spin` sums the two per-spin
1-RDM channels with high-spin (Hund) per-spin occupations (`hund_occupations`). For closed subshells
this reduces exactly to the restricted hole; for open shells it is the exact exchange hole of the
restricted/ROHF determinant (more negative than the spin-averaged hole -- e.g. N -1.61 -> -2.12 Ha).
Every converged atom with cached orbitals is included; `closed` flags the closed-subshell subset.
...
def build_atom(Z):
    ...
    if not bool(hf["converged"]) or np.asarray(hf["g_sorted"]).size == 0 or occ.size == 0:
        return None
    closed = is_exact_atom(occ, lval)
    occ_up, occ_dn = oh.hund_occupations(occ, lval)        # high-spin per-spin split (= occ/2 if closed)
    ...
    for k, i in enumerate(sel):
        u = Rad[i] * t
        nx = oh.exchange_hole_spin(float(r[i]), u, rs, gs, occ_up, occ_dn, lval, n_mu=64)
        rt0 = Rb1 @ (nx * t ** 2 * wt)
        ...
        eps_full[k] = oh.exact_eps_x_l_spin(float(r[i]), rs, gs, occ_up, occ_dn, lval, n_u=128, n_mu=64)
    ...
    return dict(Z=Z, sym=nm(Z), r0=r[sel], rho=rho[sel], Rad=Rad[sel], Q=Q[sel], s=s[sel],
                ...
                leakQ=leakQ, leakE=leakE, Ehf=Ehf, Emm=Emm, npts=len(sel), closed=closed)


def main():
    cols = ["Z", "r0", "rho", "Rad", "Q", "s", "eps_win", "eps_full", "eps_mm", "leakQ", "leakE", "closed"]
    ...
    for Z in available_hf():
        ...
        flat["closed"].append(np.full(n, b["closed"], dtype=bool))
    ...
    npts = len(flat["Z"]); nclosed = int(flat["closed"].sum())
    mae = 1e3 * np.mean(np.abs(np.array(atoms_Emm) - np.array(atoms_Ehf)))
    print(f"\nwrote {os.path.abspath(OUT)}")
    print(f"  {len(atoms_Z)} atoms ({npts} pts: {nclosed} closed-shell + {npts-nclosed} open-shell "
          f"spin-resolved); energy-reconstruction MAE {mae:.2f} mHa")
